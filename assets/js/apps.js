...
NN_FRAMEWORK.xemphongthuy = function(){
	$('.xemhuongnha').click(function(){
		var namsinh = $('#namsinh').val();
		var gioitinh = $('#gioitinh').val();
		var trend = $('#trend').val();
		$.ajax({
			url:'ajax/xemhuongnha.php',
			type:'post',
			data:{namsinh:namsinh,gioitinh:gioitinh,trend:trend},
			success:function(kq){
				$("#popup-notify #popup-notify-label").text('XEM HÆ¯á»šNG NHÃ€');
				$("#popup-notify .modal-body").html($.parseJSON(kq).con);
				$('#popup-notify').appendTo("body").modal('show');
			}
		})
		return false;
	});

	$('.xemtuoi').click(function(){
		var namsinh = $('#namsinh_1').val();
		var namxay = $('#namxay').val();

		$.ajax({
			url:'ajax/xemtuoi.php',
			type:'post',
			data:{namsinh:namsinh,namxay:namxay},
			success:function(kq){
				$("#popup-notify #popup-notify-label").text('XEM TUá»”I XÃ‚Y Dá»°NG');
				$("#popup-notify .modal-body").html($.parseJSON(kq).con);
				$('#popup-notify').appendTo("body").modal('show');
			}
		})
...
function pullRightAction () {
	if(trimStart > 0) {
		...
		var newLi = jQuery('<li>').append(jQuery('<img/>', { src:'ajax/thuoc522.png'+qStr }))
			.append(jQuery('<img/>', { src:'ajax/thuoc429.png'+qStr }))
			.append(jQuery('<img/>', { src:'ajax/thuoc388.png'+qStr }));
		jQuery('#loban-thelist').prepend(newLi);
		myScroll.refresh();
	}
}
function pullLeftAction () {    if(trimEnd < 50000) {
	jQuery('#loban-scroller').width(function(i,width){
		return width + 10000;
	});
	...
	myScroll = new iScroll('loban-wrapper', {
		...
		onRefresh: function () {
			...
			if (jQuery('#pullRight').hasClass('loading')) {
				...
				jQuery('#pullRight .pullRightLabel').html('KÃ©o qua pháº£i...');
			} else if (jQuery('#pullLeft').hasClass('loading')) {
				jQuery('#pullLeft').removeClass('loading');
				jQuery('#pullLeft .pullLeftLabel').html('KÃ©o qua trÃ¡i...');
			}
		...
		onScrollEnd: function () {
			...
			if (this.x == 0 && !jQuery('#pullRight').hasClass('flip')) {
				...
				jQuery('#pullRight .pullRightLabel').html('Tháº£ ra Ä‘á»ƒ lÃ m má»›i...');
				...
				jQuery('#pullLeft .pullLeftLabel').html('Tháº£ ra Ä‘á»ƒ lÃ m má»›i...');
			}
